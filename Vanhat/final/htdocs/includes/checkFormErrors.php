...
$eventid = $eventname = $eventtype = $datestart = $dateend = $datetext = $signupstart = $signupend = $location1 = $location2 = $icon = $genrestring = $cost = $agelimit = $beginnerfriendly = $storydesc = $infodesc = $organizername = $organizeremail = $website1 = $website2 = $status = $password = "";
$genre = array();
    ...
    if ($valid) { // Form is valid, add the event to the database
        ...
        if ($result) {
            ...
            while ($result = pg_fetch_assoc($admins)) {
                ...
                if ($ADMIN == false) {
                    sendEmail($result['email'], "admin", null, null, null);
                }
            }
